...
function PricingCard({ plan }: { plan: (typeof PLANS)[number] }) {
  ...
  return (
    ...
    >
      {plan.featured && (
        <span
          className="absolute -top-3 left-7 inline-block whitespace-nowrap bg-[color:var(--color-accent)] text-[color:var(--color-accent-fg)] font-mono text-[10px] tracking-[0.18em] uppercase px-3 py-1 rounded-full"
      ...
      <div style={{ transform: `translateZ(${headerZ}px)` }} className="font-mono text-[11px] tracking-[0.18em] uppercase text-fg-muted mb-5">
        {plan.tier}
      </div>
      ...
      >
        <span className="font-display font-extrabold text-[56px] leading-none tabular-nums">
          ${plan.monthly}
        </span>
      ...
      <p
        ...
        className="text-[14px] text-fg-muted leading-[1.6] pb-7 mb-7 border-b border-[color:var(--color-border)]"
      >
        {plan.blurb}
      </p>

      <ul style={{ transform: `translateZ(${featuresZ}px)` }} className="flex flex-col gap-3 mb-8">
        {plan.features.map((f) => (
          <li key={f.text} className="flex items-start gap-3 text-[14px]">
            {f.included ? (
              ...
              <Minus size={16} weight="bold" className="text-fg-faint shrink-0 mt-1" />
            )}
            <span className={f.included ? "text-fg" : "text-fg-faint line-through"}>{f.text}</span>
          </li>
        ))}
      </ul>
      ...
      <a
        ...
        className={`inline-flex items-center justify-center gap-2 rounded-md py-3.5 font-mono text-[12px] uppercase tracking-[0.14em] ${plan.featured ? "bg-[color:var(--color-accent)] text-[color:var(--color-accent-fg)] hover:bg-[color:var(--color-accent-hover)]" : "border border-[color:var(--color-border-strong)] hover:border-[color:var(--color-accent)] hover:text-[color:var(--color-accent)]"}`}
      >
